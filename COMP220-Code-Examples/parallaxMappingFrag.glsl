...
vec2 CalculateParallaxTextureCoords(vec2 texCoords,vec3 viewDir)
{
	float height=texture(heightTexture,texCoords).r;
	vec2 adjustedTexCoords=texCoords-(viewDir.xy/viewDir.z*(height*0.1));
...
void main()
{
	vec2 parallaxTexCoords=CalculateParallaxTextureCoords(vertexTextureCoordOut,viewDirection);

	vec4 diffuseTextureColour=texture(diffuseTexture,parallaxTexCoords);

	vec4 specularTextureColour=texture(specularTexture,parallaxTexCoords);
	
	vec3 normal=texture(normalTexture,parallaxTexCoords).rgb;
